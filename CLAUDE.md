...
- **Compression strategy**: when the core is constructed with `compress=true`, PNG/GIF inputs are re-encoded as JPEG (quality 75, `ImageCompressionQuality`) — inline parts in memory, File API uploads streamed through `io.Pipe`. Other formats pass through untouched.
