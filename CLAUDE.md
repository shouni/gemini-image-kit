...
- **Request guards live in `GeminiGenerator`** (`WithRateLimit`, `WithMaxConcurrency`, `WithRequestTimeout`): before v1.13 every consumer (go-veo-orchestrator, go-comic-kit) re-implemented rate limiting + errgroup around the kit. `GenerateBatch` preserves order, **keeps partial successes on failure** (each image already cost money), and reports failures via `errors.Join`.
